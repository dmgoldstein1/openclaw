...
// Generated by Copilot (GPT-5.3-Codex): Discover LM Studio models from OpenAI-compatible /v1/models.
...
function resolveOpenAiModelsEndpoint(baseUrl?: string): string {
  const normalized = (baseUrl?.trim() || LMSTUDIO_DEFAULT_BASE_URL).replace(/\/+$/, "");
...
function resolveDiscoveryApiKey(value?: string): string {
  const normalized = value?.trim() ?? "";
  if (!normalized) {
    return "";
  }
  if (/^[A-Z][A-Z0-9_]*$/.test(normalized)) {
    return (process.env[normalized] ?? "").trim();
  }
  return normalized;
}

export async function discoverLmstudioModels(params?: {
  baseUrl?: string;
  apiKey?: string;
}): Promise<ModelDefinitionConfig[]> {
  if (process.env.VITEST || process.env.NODE_ENV === "test") {
  ...
  const endpoint = resolveOpenAiModelsEndpoint(params?.baseUrl);
  const resolvedApiKey = resolveDiscoveryApiKey(params?.apiKey);
  const headers =
    resolvedApiKey !== ""
      ? {
...
async function buildLmstudioProvider(params?: {
  ...
  const baseUrl = (params?.baseUrl?.trim() || LMSTUDIO_DEFAULT_BASE_URL).replace(/\/+$/, "");
...
export async function resolveImplicitProviders(params: {
  ...
  const openaiBaseFromEnv = process.env.OPENAI_API_BASE?.trim() ?? "";
  const lmstudioBaseUrl =
    explicitLmstudio?.baseUrl?.trim() ||
    openaiBaseFromEnv ||
    params.explicitProviders?.openai?.baseUrl?.trim() ||
    "";
  if (lmstudioBaseUrl) {
    const explicitLmstudioApiKey = normalizeOptionalSecretInput(explicitLmstudio?.apiKey);
    const lmstudioApiKey =
      explicitLmstudioApiKey ??
    ...
    if (lmstudioProvider.models.length > 0 || explicitLmstudio) {
      providers.lmstudio = {
        ...lmstudioProvider,
